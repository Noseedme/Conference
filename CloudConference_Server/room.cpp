...
typedef struct pool
{
    ...
    int num;
    int status[1024 + 10];
    std::map<int, uint32_t> fdToIp;//存储文件描述符与IP地址的映射关系
    pool()
    {
...
void process_main(int i, int fd) // room start
{
    ...
    *ptr = fd;
    Pthread_create(&pfd1, NULL, accept_fd, ptr); //创建一个新线程用于接收连接的文件描述符
...
void fdclose(int fd, int pipefd)
{
    ...
    else
    {
        ...
        FD_CLR(fd, &user_pool->fdset);//清除fd
        user_pool->num--;
