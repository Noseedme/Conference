    ...
    : QWidget{parent}
{
    ...
    m_leftPixmap = QPixmap(":/score/1.png");
    m_rightPixmap = QPixmap(":/score/1.png");

    m_loadingMovie = new QMovie(this);
    m_loadingMovie->setFileName(":/score/3.gif");
...
QSize ChatMessage::fontRect(QString str)
{
    ...
    int minHei = 30;//最小高度
    int iconWH = 40;//图标宽高
    int iconSpaceW = 20;//图标间距
    int iconRectW = 5;//图标矩形宽度
    int iconTMPH = 10;//图标临时高度
    int sanJiaoW = 6;//sanJiaoW
    int kuangTMP = 20;//框临时参数
    int textSpaceRect = 12;//文本与矩形间距
    //消息框的宽度
...
void ChatMessage::paintEvent(QPaintEvent *event)
{
    ...
    painter.setBrush(QBrush(Qt::gray));//填充颜色为灰色的矩形
    ...
    if(m_userType == User_Type::User_She) { // 用户
        ...
        //框加边
        QColor col_KuangB(234, 234, 234);//白色
